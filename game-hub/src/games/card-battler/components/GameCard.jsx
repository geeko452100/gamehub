...
export default function GameCard({ card, onPlay, canAfford }) {
  return (
    <div 
      className={`w-40 h-56 bg-slate-950 border rounded-xl p-3 flex flex-col justify-between shadow-2xl transition-all duration-200 select-none
        ${canAfford 
          ? 'border-indigo-500/30 cursor-pointer hover:-translate-y-4 hover:shadow-indigo-500/10 hover:border-indigo-400' 
          : 'border-slate-800 opacity-50 cursor-not-allowed'
        }`}
      onClick={() => canAfford && onPlay(card.id)}
    >
      {/* Card Header: Name and Energy Cost */}
